...
def user_has_write_permission(path, user):
    ...
    with Connection.create(ldap_servers) as conn:
        if mode & 0o020:
            ...
            group = fstat.st_gid
            query = Query(conn, base_dn=settings.JDMA_LDAP_BASE_GROUP).filter(gidNumber=group)

            # check for a valid return
            if len(query) == 0:
                logging.error("Group with gidNumber: {} not found from LDAP".format(group))
                return False
        ...
        if mode & 0o200:
            ...
            query = Query(conn, base_dn=settings.JDMA_LDAP_BASE_USER).filter(uid=user)
            if len(query) == 0:
                logging.error("Unix user id: {} not found from LDAP in UserHasWritePermission".format(user))
                return False
